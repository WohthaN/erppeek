#!/usr/bin/env python
# -*- coding: utf-8 -*-
""" erppeek.py -- zelo_core / OpenERP client library and command line tool
    ...
    if issubclass(exc_type, Error):             # Client-side
        ...
        if warning:
            ...
            if parts[-1] == 'None':
                ...
                if last_line.startswith('zelo_core.exceptions'):
...
class Service(object):
    ...
    If `server` is an ``zelo_core`` Python package, it is used to connect to the
...
class Client(object):
    """Connection to an zelo_core instance.

    This is the top level object.
    The `server` is the URL of the instance, like ``http://localhost:8069``.
    If `server` is an ``zelo_core``/``openerp`` Python package, it is used to
    ...
    def login(self, user, password=None, database=None):
        ...
        if database:
            ...
            if not self._db:
                self._db = database
            # Used for logging, copied from zelo_core.sql_db.db_connect
    ...
    def _set_interactive(cls, global_vars={}):
        ...
        def get_pool(db_name=None):
            ...
            Use get_pool().cursor() to grab a cursor on an zelo_core database.
...
class Model(object):
    """The class for zelo_core models."""
    ...
    def create(self, values, context=_DEFAULT):
        ...
        accept either a list of ids or a RecordList or the extended zelo_core
        syntax.  Relationship fields `many2one` and `reference` accept
        either a Record or the zelo_core syntax.
...
class RecordList(object):
    """A sequence of zelo_core :class:`Record`.
...
class Record(object):
    """A class for all zelo_core records.

    It maps any zelo_core object.
...
def main(interact=_interact):
    description = ('Inspect data on zelo_core objects.  Use interactively '
